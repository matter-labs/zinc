...
use franklin_crypto::circuit::{num::AllocatedNum, boolean::Boolean, Assignment};
use franklin_crypto::circuit::boolean::AllocatedBit;

fn or<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, left: &Boolean, right: &Boolean) -> Result<Boolean, SynthesisError> {
    let boolean = Boolean::and(
        cs.namespace(|| "and"),
        &left.not(),
        &right.not(),
    )?.not();
        ...
        CS: ConstraintSystem<E>
{
    ...
    Ok(z_bits.last().expect("bit representation of z can't be empty").clone())
}
        ...
        CS: ConstraintSystem<E>
{
    ...
    let a_lower = AllocatedNum::pack_bits_to_element(cs.namespace(|| "a_lower"), &bits_a[..lower_bits_len])?;
    let b_lower = AllocatedNum::pack_bits_to_element(cs.namespace(|| "b_lower"), &bits_b[..lower_bits_len])?;

    let a_upper = AllocatedNum::pack_bits_to_element(cs.namespace(|| "a_upper"), &bits_a[lower_bits_len..])?;
    let b_upper = AllocatedNum::pack_bits_to_element(cs.namespace(|| "b_upper"), &bits_b[lower_bits_len..])?;

    let upper_a_lt_b = less_than_fixed(cs.namespace(|| "upper_a_lt_b"), &a_upper, &b_upper, upper_bits_len)?;
    let lower_a_lt_b = less_than_fixed(cs.namespace(|| "lower_a_lt_b"), &a_lower, &b_lower, lower_bits_len)?;
    let upper_a_eq_b = AllocatedNum::equals(cs.namespace(|| "upper_a_eq_b"), &a_upper, &b_upper)?;

    let lower_lt_and_upper_eq = Boolean::and(cs.namespace(|| ""), &lower_a_lt_b, &upper_a_eq_b.into())?;
