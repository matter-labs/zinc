...
impl Scope {
    ...
    pub fn declare_input(&mut self, input: Input) -> Result<(), Error> {
        if self.is_variable_declared(input.identifier()) {
            return Err(Error::VariableAlreadyDeclared(
                input.identifier().name().to_owned(),
            ));
        }
        let place = Place::new(
        ...
        self.variables
            .insert(input.identifier().name().to_owned(), place);
        Ok(())
    }
    ...
    pub fn declare_witness(&mut self, witness: Witness) -> Result<(), Error> {
        if self.is_variable_declared(witness.identifier()) {
            return Err(Error::VariableAlreadyDeclared(
                witness.identifier().name().to_owned(),
            ));
        }
        let place = Place::new(
        ...
        self.variables
            .insert(witness.identifier().name().to_owned(), place);
        Ok(())
    }
    ...
    pub fn declare_variable(&mut self, place: Place) -> Result<(), Warning> {
        let result = if self.is_variable_declared(&place.identifier) {
            Err(Warning::RedeclaredVariable(
