...
) -> Result<Vec<AllocatedBit>, SynthesisError>
{
    ...
    let bits = values.into_iter().rev().enumerate().take(bit_length).map(|(i, b)| {
        AllocatedBit::alloc(
...
pub fn into_bits_le_fixed<CS>(
        ...
        where CS: ConstraintSystem<Bn256>
    {
        let bits = field_into_allocated_bits_le_fixed(
            &mut cs,
            number.get_value(),
            bit_length
        )?;
...
pub fn alloc_input<CS: ConstraintSystem<Bn256>, F: FnOnce() -> Result<Fr, SynthesisError>>(mut cs: CS, a: F, bit_length: usize) -> Result<(AllocatedNum<Bn256>, Vec<Boolean>), SynthesisError>{
...
pub fn alloc_witness<CS: ConstraintSystem<Bn256>, F: FnOnce() -> Result<Fr, SynthesisError>>(mut cs: CS, a: F, bit_length: usize) -> Result<(AllocatedNum<Bn256>, Vec<Boolean>), SynthesisError>{
    let number = AllocatedNum::alloc(cs.namespace(||"allocate_number"), a)?;
    let bits = into_bits_le_fixed(cs.namespace(||"input_bits"), &number, bit_length)?;
    Ok((number, bits))
}
...
pub fn sum<CS: ConstraintSystem<Bn256>>(mut cs: CS, a: &AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>, bit_length: usize) -> Result<(AllocatedNum<Bn256>, Vec<Boolean>), SynthesisError>{
...
pub fn sum_unchecked<CS: ConstraintSystem<Bn256>>(mut cs: CS, a: &AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>) -> Result<AllocatedNum<Bn256>, SynthesisError>{
    let sum = AllocatedNum::alloc(cs.namespace(||"name_first+name_second"), ||{
        let mut sum = a.get_value().unwrap();
        sum.add_assign(&b.get_value().unwrap());
        Ok(sum)
...
// #jab: a-b transpiles into this
pub fn diff<CS: ConstraintSystem<Bn256>>(mut cs: CS, a: &AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>, bit_length: usize) -> Result<(AllocatedNum<Bn256>, Vec<Boolean>), SynthesisError>{
...
// #jab: a*b transpiles into this
pub fn mul<CS: ConstraintSystem<Bn256>>(mut cs: CS, a: &AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>, bit_length: usize) -> Result<(AllocatedNum<Bn256>, Vec<Boolean>), SynthesisError>{
...
pub fn greater<CS: ConstraintSystem<Bn256>>(mut cs: CS, a: &AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>, bit_length: usize) -> Result<Boolean, SynthesisError>{
        let (diff_a_b, diff_bits) = diff(cs.namespace(||"a-b"), a, b, bit_length)?;
        let diff_a_b_repacked =
            pack_bits_to_element(cs.namespace(|| "pack a-b bits"), &diff_bits)?;
...
// #jab: a<b transpiles into this //TODO: this is actually a>=b 
pub fn less<CS: ConstraintSystem<Bn256>>(mut cs: CS, a: &AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>, bit_length: usize) -> Result<Boolean, SynthesisError>{
        greater(cs, b, a, bit_length)
}

// #jab: a==b transpiles into this //TODO: this is actually a>=b 
pub fn equals<CS: ConstraintSystem<Bn256>>(mut cs: CS, a: &AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>, bit_length: usize) -> Result<Boolean, SynthesisError>{
        Ok(Boolean::from(AllocatedNum::equals(cs.namespace(||"equals"), a, b)?))
}
...
pub fn if_else_example<CS: ConstraintSystem<Bn256>>(mut cs: CS, mut a: AllocatedNum<Bn256>, b: &AllocatedNum<Bn256>, c: &AllocatedNum<Bn256>, cond: &Boolean)->Result<(), SynthesisError>{
    a = AllocatedNum::conditionally_select(cs.namespace(||"select first if"), &a, b, cond)?;
    a = AllocatedNum::conditionally_select(cs.namespace(||"select first else"), &a, c, &cond.not())?;
    Ok(())
}
