...
pub fn decode_instruction(bytes: &[u8]) -> Result<(Instruction, usize), DecodingError> {
    ...
    match bytes[0] {
        ...
        x if x == InstructionCode::LoadArray as u8 => decode_and_wrap::<LoadArray>(bytes),
        x if x == InstructionCode::StoreArray as u8 => decode_and_wrap::<StoreArray>(bytes),
        ...
        x if x == InstructionCode::LoadArrayByIndex as u8 => decode_and_wrap::<LoadArrayByIndex>(bytes),
        x if x == InstructionCode::StoreArrayByIndex as u8 => decode_and_wrap::<StoreArrayByIndex>(bytes),
