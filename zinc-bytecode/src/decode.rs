...
pub fn decode_instruction(bytes: &[u8]) -> Result<(Instruction, usize), DecodingError> {
    ...
    match bytes[0] {
        ...
        x if x == InstructionCode::RefStore as u8 => decode_and_wrap::<RefStore>(bytes),
        x if x == InstructionCode::RefStoreSequence as u8 => decode_and_wrap::<RefStoreSequence>(bytes),
