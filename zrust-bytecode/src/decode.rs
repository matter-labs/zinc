...
pub fn decode_instruction(bytes: &[u8]) -> Result<(Instruction, usize), DecodingError> {
    ...
    match bytes[0] {
        ...
        x if x == InstructionCode::Push as u8 => decode_and_wrap::<Push>(bytes),
        ...
        x if x == InstructionCode::FrameBegin as u8 => decode_and_wrap::<LoopBegin>(bytes),
        x if x == InstructionCode::FrameEnd as u8 => decode_and_wrap::<LoopBegin>(bytes),
